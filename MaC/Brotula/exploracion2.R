...
datos$catch   <- datos$catch_brotola/1000
datos$cpue    <- datos$catch/datos$h_a
datos$prop    <- datos$catch_brotola/datos$captura.total
...
## --------------- sorting data ----------------------------
data.cojinoba     <- datos  %>%  filter(catch>0 & catch<10) %>% ##%>% filter( zona=='ZN' | zona=='ZS') 
...
dat.cojinoba.ope <- data.cojinoba %>% select(c(1:26,62:82))
names(dat.cojinoba.ope)
dat.cojinoba.esp <- data.cojinoba %>% select(c(27:61))
...
#write.csv(data.cojinoba, file = "data_brotula_arrastre_2021.csv", row.names = T)
...
dat.cojinoba.ope <- data.cojinoba %>% select(c(1:27,63:83))
names(dat.cojinoba.ope)
dat.cojinoba.esp <- data.cojinoba %>% select(c(28:62))
dat.cojinoba.esp <- dat.cojinoba.esp %>% mutate(total = rowSums(.,na.rm=TRUE))
names(dat.cojinoba.esp)
