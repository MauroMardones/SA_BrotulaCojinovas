...
nb2 <- ggplot(tt , aes(x=Long, y = as.factor(Year), fill=Sex))  +
  ...
  scale_x_discrete(breaks = seq(from = 20, to = 90, by = 10))+
...
```{r}
tt$cat_long <- as.numeric(as.character(cut(x = tt$Long, breaks = seq(10,108,2), 
...
```{r tallasCojinobaMoteada, warning=F, message=F, fig.width=3, fig.height=4}
