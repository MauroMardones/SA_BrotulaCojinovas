...
tserie <- data.cojinoba %>% tibble::as_tibble() %>%    #pesquería 5= flota merlucera (IFOP), 10= palangre hielero (IFOP) y 11=palangre fábrica (IFOP).
  select(añoLance, h_a, catch_cojinoba, NOMBRE_BARCO) %>% 
  filter(h_a>0, catch_cojinoba > 0, añoLance>=1997)#  filter(!(year==2015 &  COD_BARCO==400075)) 
dim(tserie)
#18367 4 
#19688 4
#26710 4 
...
#######################################################################3
ncatch1 <- tserie %>% group_by(añoLance,NOMBRE_BARCO) %>% 
